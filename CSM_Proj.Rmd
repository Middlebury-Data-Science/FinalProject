...
###Principal Compoenent Analsysis
...
The main objective of this project is to identify bacteria that may be preventing *Borrelia burgdorferi* (the Lyme disease bactiera) from colonizing a tick. I do this now by comparing the microbiomes of each and trying to find OTUs that are associated with a reduction in *Borrelia* prevalence. Below are two sets of stacked bar charts, negative ticks on top, and positives on bottom. In this view no major differences really stand out, except for the obvious that postives have the orange *Borrelia* OTU. Still, it's worth showing the stacked format again in case anything obvious did pop out.
...
Here we see that the 4 most important OTUs are all closely related, in the family *Oxalobacteraceae* (the genus *Jathinobacterium* is in this family). This suggests that these correlations might be the result of an actual biological interaction rathern than random noise, as similar species are showing a similar pattern.

Unfortunately for us, these species are all positively correlated with *Borrelia* abundance. This means that even if it represents a biologically relevant interaction, these *Oxalobacteraceae* are actually fascilitating *Borrelia* colonization, rather than preventing it.
